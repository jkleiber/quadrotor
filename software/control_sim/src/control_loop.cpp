...
bool ControlLoop::UpdateParams(bool enabled)
{
    ...
    if (ImGui::BeginTabItem("Controllers"))
    {
        ...
        {
            ImGui::InputScalar("Roll Kp", ImGuiDataType_Double, &roll_gains.kp,
                               NULL);
            ImGui::InputScalar("Roll Ki", ImGuiDataType_Double, &roll_gains.ki,
                               NULL);
            ImGui::InputScalar("Roll Kd", ImGuiDataType_Double, &roll_gains.kd,
                               NULL);
            ImGui::InputScalar("Roll Min Output", ImGuiDataType_Double,
                               &roll_gains.min_out, NULL);
            ImGui::InputScalar("Roll Max Output", ImGuiDataType_Double,
                               &roll_gains.max_out, NULL);
        }
        if (ImGui::CollapsingHeader("Pitch PID"))
        {
            ImGui::InputScalar("Pitch Kp", ImGuiDataType_Double,
                               &pitch_gains.kp, NULL);
            ImGui::InputScalar("Pitch Ki", ImGuiDataType_Double,
                               &pitch_gains.ki, NULL);
            ImGui::InputScalar("Pitch Kd", ImGuiDataType_Double,
                               &pitch_gains.kd, NULL);
            ImGui::InputScalar("Pitch Min Output", ImGuiDataType_Double,
                               &pitch_gains.min_out, NULL);
            ImGui::InputScalar("Pitch Max Output", ImGuiDataType_Double,
                               &pitch_gains.max_out, NULL);
        }
        if (ImGui::CollapsingHeader("Yaw PID"))
        {
            ImGui::InputScalar("Yaw Kp", ImGuiDataType_Double, &yaw_gains.kp,
                               NULL);
            ImGui::InputScalar("Yaw Ki", ImGuiDataType_Double, &yaw_gains.ki,
                               NULL);
            ImGui::InputScalar("Yaw Kd", ImGuiDataType_Double, &yaw_gains.kd,
                               NULL);
            ImGui::InputScalar("Yaw Min Output", ImGuiDataType_Double,
                               &yaw_gains.min_out, NULL);
            ImGui::InputScalar("Yaw Max Output", ImGuiDataType_Double,
                               &yaw_gains.max_out, NULL);
        }
        if (ImGui::CollapsingHeader("Height PID"))
        {
            ImGui::InputScalar("Height Kp", ImGuiDataType_Double,
                               &height_gains.kp, NULL);
            ImGui::InputScalar("Height Ki", ImGuiDataType_Double,
                               &height_gains.ki, NULL);
            ImGui::InputScalar("Height Kd", ImGuiDataType_Double,
                               &height_gains.kd, NULL);
            ImGui::InputScalar("Height Kf", ImGuiDataType_Double,
                               &height_gains.kf, NULL);
            ImGui::InputScalar("Height Min Output", ImGuiDataType_Double,
                               &height_gains.min_out, NULL);
            ImGui::InputScalar("Height Max Output", ImGuiDataType_Double,
                               &height_gains.max_out, NULL);
        }
        ImGui::EndTabItem();
    }
...
void ControlLoop::SaveGains()
{
    std::string gain_string = ""
                              "// Roll PID\n"
                              "roll_gains.kp = " +
                              std::to_string(roll_gains.kp) +
                              ";\n"
                              "roll_gains.ki = " +
                              std::to_string(roll_gains.ki) +
                              ";\n"
                              "roll_gains.kd = " +
                              std::to_string(roll_gains.kd) +
                              ";\n"
                              "roll_gains.kf = " +
                              std::to_string(roll_gains.kf) +
                              ";\n"
                              "roll_gains.min_out = " +
                              std::to_string(roll_gains.min_out) +
                              ";\n"
                              "roll_gains.max_out = " +
                              std::to_string(roll_gains.max_out) +
                              ";\n"
                              "\n"
                              "// Pitch PID\n"
                              "pitch_gains.kp = " +
                              std::to_string(pitch_gains.kp) +
                              ";\n"
                              "pitch_gains.ki = " +
                              std::to_string(pitch_gains.ki) +
                              ";\n"
                              "pitch_gains.kd = " +
                              std::to_string(pitch_gains.kd) +
                              ";\n"
                              "pitch_gains.kf = " +
                              std::to_string(pitch_gains.kf) +
                              ";\n"
                              "pitch_gains.min_out = " +
                              std::to_string(pitch_gains.min_out) +
                              ";\n"
                              "pitch_gains.max_out = " +
                              std::to_string(pitch_gains.max_out) +
                              ";\n"
                              "\n"
                              "// Yaw PID\n"
                              "yaw_gains.kp = " +
                              std::to_string(yaw_gains.kp) +
                              ";\n"
                              "yaw_gains.ki = " +
                              std::to_string(yaw_gains.ki) +
                              ";\n"
                              "yaw_gains.kd = " +
                              std::to_string(yaw_gains.kd) +
                              ";\n"
                              "yaw_gains.kf = " +
                              std::to_string(yaw_gains.kf) +
                              ";\n"
                              "yaw_gains.min_out = " +
                              std::to_string(yaw_gains.min_out) +
                              ";\n"
                              "yaw_gains.max_out = " +
                              std::to_string(yaw_gains.max_out) +
                              ";\n"
                              "\n"
                              "// Height PID\n"
                              "height_gains.kp = " +
                              std::to_string(height_gains.kp) +
                              ";\n"
                              "height_gains.ki = " +
                              std::to_string(height_gains.ki) +
                              ";\n"
                              "height_gains.kd = " +
                              std::to_string(height_gains.kd) +
                              ";\n"
                              "height_gains.kf = " +
                              std::to_string(height_gains.kf) +
                              ";\n"
                              "height_gains.min_out = " +
                              std::to_string(height_gains.min_out) +
                              ";\n"
                              "height_gains.max_out = " +
                              std::to_string(height_gains.max_out) + ";\n";

    // Save this string to the gains file (to load as default via copy-paste
    // later.) Gains export to TOML.
